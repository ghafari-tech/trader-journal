...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "TraderJournal AI — ژورنال هوشمند معامله‌گران بازارهای مالی" },
      {
        name: "description",
        content:
...
function Landing() {
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-40 border-b border-border/60 bg-background/70 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 md:px-8">
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="font-bold">TraderJournal <span className="text-primary">AI</span></span>
          </Link>
            ...
            <div className="card-surface overflow-hidden rounded-2xl p-1">
              <div className="rounded-xl bg-background/40 p-6">
                <div className="grid gap-4 md:grid-cols-4">
                  {[
                    { label: "سود کل", value: "$۱۲,۴۸۰", change: "+۲۴.۸٪", positive: true },
                    { label: "Win Rate", value: "۶۸٪", change: "+۴.۲٪", positive: true },
                    { label: "Profit Factor", value: "۲.۱۴", change: "+۰.۳", positive: true },
                    { label: "Max Drawdown", value: "-۸.۲٪", change: "-۱.۱٪", positive: false },
                  ].map((s) => (
                    <div key={s.label} className="rounded-lg border border-border bg-card p-4">
                      <div className="text-xs text-muted-foreground">{s.label}</div>
                      <div className="mt-2 text-2xl font-bold tabular">{s.value}</div>
                      <div className={`mt-1 text-xs tabular ${s.positive ? "gain" : "loss"}`}>{s.change}</div>
                    </div>
                  ))}
                </div>
      ...
      {/* How it works */}
      <section id="how" className="border-t border-border bg-secondary/20 py-20">
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          <div className="mx-auto max-w-2xl text-center">
      ...
      {/* FAQ */}
      <section id="faq" className="border-t border-border bg-secondary/20 py-20">
      ...
      <footer className="border-t border-border py-8">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 text-sm text-muted-foreground md:flex-row md:px-8">
          <div className="flex items-center gap-2">
            ...
            <span>© ۱۴۰۳ TraderJournal AI — تمام حقوق محفوظ است.</span>
          </div>
