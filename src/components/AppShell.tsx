...
const notifications = [
  { id: 1, icon: AlertTriangle, tone: "loss", title: "نزدیک به سقف ریسک روزانه", desc: "به ۸۰٪ ریسک روزانه رسیدی.", time: "۵ دقیقه پیش" },
  { id: 2, icon: BookOpen, tone: "primary", title: "یادآور ژورنال", desc: "برای معامله T-1042 ژورنال ثبت نکردی.", time: "۱ ساعت پیش" },
  { id: 3, icon: Trophy, tone: "accent", title: "نشان جدید کسب کردی", desc: "«۷ روز پایبند به پلن» فعال شد.", time: "دیروز" },
  { id: 4, icon: CheckCircle2, tone: "primary", title: "گزارش هفتگی آماده است", desc: "مربی هوشمند گزارش هفتگی‌ات را ساخت.", time: "۲ روز پیش" },
];
      ...
      <DropdownMenuTrigger asChild>
        ...
        >
          <Avatar className="h-9 w-9">
            <AvatarFallback className="bg-primary/20 text-primary text-xs font-bold">ع.ر</AvatarFallback>
          </Avatar>
          <div className="min-w-0 flex-1">
            <div className="truncate text-sm font-medium">علی رضایی</div>
...
function NotificationsMenu() {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="icon" className="relative h-10 w-10 border-border bg-secondary/60">
      ...
      <DropdownMenuContent align="end" className="w-80">
        ...
        <div className="max-h-80 overflow-y-auto">
          {notifications.map((n) => {
            const Icon = n.icon;
            return (
              <DropdownMenuItem key={n.id} className="cursor-pointer items-start gap-3 py-2.5">
                <div
                  className={cn(
                    "mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg",
                    n.tone === "loss" && "bg-destructive/15 text-destructive",
                    n.tone === "primary" && "bg-primary/15 text-primary",
                    n.tone === "accent" && "bg-accent/15 text-accent",
                  )}
                >
        ...
        <aside className="hidden w-64 shrink-0 border-l border-sidebar-border bg-sidebar lg:flex lg:flex-col">
          <div className="flex h-16 items-center gap-2 border-b border-sidebar-border px-5">
            ...
            <div className="flex flex-col leading-tight">
              <span className="text-sm font-bold">TraderJournal</span>
